...
public class PropertyRepeatabilityController extends AnchorPane{
...
/*		if((ConstantApp.USER_ACCESS_LEVEL.equals(ConstantApp.TESTER_ACCESS_LEVEL)) ||
	...
	private void applyUacSettings() {
		...
		ApplicationLauncher.logger.debug("PropertyRepeatabilityController:  applyUacSettings: Entry");
	...
	public static  void PropertyDisplayUpdate(String testType, String aliasId, DraggableTestNode SelectedNode){
		...
		if(mCurrentNode.IsNewNode() != true){
			...
			try {
				...
				e.printStackTrace();
				ApplicationLauncher.logger.error("PropertyRepeatabilityController: PropertyDisplayUpdate: JSONException:"+e.getMessage());
	...
	public void SaveToDB(){
		...
		if(columnData.size() !=0){
			...
			int no_of_test_points = Integer.parseInt(no_of_readings)*columnData.size();
			...
			if(saved_successfully_count == no_of_test_points){
				ApplicationLauncher.logger.info ("PropertyRepeatabilityController: SaveToDB: Test point saved successfully");
				ApplicationLauncher.InformUser("Saved", "Test point saved successfully",AlertType.INFORMATION);
			}
			else{
				ApplicationLauncher.logger.info ("PropertyRepeatabilityController: SaveToDB: Test point save failed. Test point save failed:" +saveFailedReason);
				ApplicationLauncher.InformUser("Incorrect Value", "Test point save failed: " + saveFailedReason,AlertType.ERROR);

			}
		}
		else{
			ApplicationLauncher.logger.info ("PropertyRepeatabilityController: SaveToDB:  Save the test point inputs - warning triggered");
...
/*	public void InformUser(String title, String info,AlertType Alert_type){
	...
	public void SaveStdDataTrigger(){
		...
		ApplicationLauncher.logger.info("PropertyRepeatabilityController : SaveStdDataTrigger Invoked:");
